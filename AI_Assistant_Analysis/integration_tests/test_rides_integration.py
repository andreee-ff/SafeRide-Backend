...
class TestRideEndpoints:
    ...
    async def test_owned_rides_empty(self, client, auth_headers):
        """Test /rides/owned returns empty list if user has no rides"""
        response = await client.get("/rides/owned", headers=auth_headers)
        assert response.status_code == 404 or (response.status_code == 200 and response.json() == [])

    async def test_owned_rides_not_visible_to_others(self, client, auth_headers, second_user_headers):
        ...
        await client.post("/rides/", json=ride, headers=auth_headers)
        response = await client.get("/rides/owned", headers=second_user_headers)
        assert response.status_code == 404 or (response.status_code == 200 and all(r["title"] != "Private Ride" for r in response.json()))

    async def test_joined_rides_empty(self, client, second_user_headers):
        """Test /rides/joined returns empty list if user is not a participant"""
        response = await client.get("/rides/joined", headers=second_user_headers)
        assert response.status_code == 404 or (response.status_code == 200 and response.json() == [])

    async def test_joined_rides_only_participating(self, client, auth_headers, second_user_headers):
        ...
        r1_resp = await client.post("/rides/", json=ride1, headers=auth_headers)
        r1 = r1_resp.json()
        await client.post("/rides/", json=ride2, headers=auth_headers)
        
        # User 2 joins only the first ride
        join_payload = {"ride_code": r1["code"]}
        join_response = await client.post(f"/participations/", json=join_payload, headers=second_user_headers)
    ...
    async def test_joined_rides_after_leaving(self, client, auth_headers, second_user_headers):
        ...
        r_resp = await client.post("/rides/", json=ride, headers=auth_headers)
        r = r_resp.json()
        ...
        # User 2 leaves
        leave_response = await client.delete(f"/participations/{participation_id}", headers=second_user_headers)
    ...
    async def test_owned_rides_deleted_not_returned(self, client, auth_headers):
        ...
        await client.delete(f"/rides/{r['id']}", headers=auth_headers)
        response = await client.get("/rides/owned", headers=auth_headers)
        assert response.status_code == 404 or (response.status_code == 200 and all(rr["title"] != "To Delete" for rr in response.json()))

    async def test_owned_and_joined_requires_auth(self, client):
        """Test /rides/owned and /rides/joined require authentication"""
        response_owned = await client.get("/rides/owned")
        response_joined = await client.get("/rides/joined")
        assert response_owned.status_code == 401
        assert response_joined.status_code == 401

    async def test_get_owned_rides(self, client, auth_headers, second_user_headers):
        ...
        await client.post("/rides/", json=ride1, headers=auth_headers)
        await client.post("/rides/", json=ride2, headers=auth_headers)
        ...
        await client.post("/rides/", json=ride3, headers=second_user_headers)
    ...
    async def test_get_joined_rides(self, client, auth_headers, second_user_headers):
        """Test /rides/joined returns only rides where user is a participant"""
        ...
        create_response = await client.post("/rides/", json=ride, headers=auth_headers)
    ...
    async def test_create_ride(self, client, auth_headers):
    ...
    async def test_create_ride_unauthorized(self, client):
        ...
        response = await client.post("/rides/", json=ride_data)
        assert response.status_code == 401
    
    async def test_get_all_rides(self, client, auth_headers):
        ...
        for ride in rides:
            response = await client.post("/rides/", json=ride, headers=auth_headers)
            assert response.status_code == 201
        
        # Get all rides
        response = await client.get("/rides/")
    ...
    async def test_get_ride_by_id(self, client, auth_headers):
        ...
        create_response = await client.post("/rides/", json=ride_data, headers=auth_headers)
        ride_id = create_response.json()["id"]
        
        # Get ride by ID
        response = await client.get(f"/rides/{ride_id}")
    ...
    async def test_get_ride_by_code(self, client, auth_headers):
        ...
        # Get ride by code
        response = await client.get(f"/rides/code/{ride_code}")
    ...
    async def test_get_nonexistent_ride(self, client):
        """Test getting ride that doesn't exist"""
        response = await client.get("/rides/99999")
        assert response.status_code == 404
    
    async def test_update_ride_by_owner(self, client, auth_headers):
        ...
        response = await client.put(f"/rides/{ride_id}", json=update_data, headers=auth_headers)
    ...
    async def test_update_ride_by_non_owner(self, client, auth_headers, second_user_headers):
        ...
        response = await client.put(f"/rides/{ride_id}", json=update_data, headers=second_user_headers)
        
        assert response.status_code == 403
    
    async def test_delete_ride_by_owner(self, client, auth_headers):
        ...
        # Delete ride
        response = await client.delete(f"/rides/{ride_id}", headers=auth_headers)
        
        assert response.status_code == 204
        
        # Verify ride is deleted
        get_response = await client.get(f"/rides/{ride_id}")
        assert get_response.status_code == 404
    
    async def test_delete_ride_by_non_owner(self, client, auth_headers, second_user_headers):
    ...
    async def test_delete_nonexistent_ride(self, client, auth_headers):
        """Test deleting ride that doesn't exist"""
        response = await client.delete("/rides/99999", headers=auth_headers)
        assert response.status_code == 404
